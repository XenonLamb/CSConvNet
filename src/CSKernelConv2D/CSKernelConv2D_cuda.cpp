...
#include "CSKernelConv2D_kernel.h"
*/

#include <torch/torch.h>
#include <ATen/ATen.h>
//#include <ATen/cuda/CUDAContext.h>
#include <ATen/Context.h> //for torch 0.4.0
...
int CSKernelConv2D_forward_cuda(
    ...
    int success = CSKernelConv2D_forward_cuda_kernel(
        input,
        kernel_bank,
        kernel_size,
        output,
       ...
       at::globalContext().getCurrentCUDAStream()  //for torch 0.4.0
    );
    if (!success) {
...
int CSKernelConv2D_backward_cuda(
    ...
    int success = CSKernelConv2D_backward_cuda_kernel(
        input,
        kernel_bank,
        kernel_size,
        grad_output,
