...
class SIDD(srdata.SRData):
    def __init__(self, args, name='SIDD', train=True, benchmark=True):
        data_range = [r.split('-') for r in args.data_range_sidd.split('/')]
        if train:
    ...
    def _scan(self,half_mode='full'):
        names_hr, names_lr = super(SIDD, self)._scan(half_mode='full')
    ...
    def _set_filesystem(self, dir_data):
        super(SIDD, self)._set_filesystem(dir_data)
