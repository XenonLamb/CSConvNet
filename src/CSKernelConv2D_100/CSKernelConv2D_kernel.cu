...
int CSKernelConv2D_backward_cuda_kernel(
    ...
    CSKernelConv2D_backward_function_input<<< (n_grad_input + THREAD_PER_BLOCK - 1) / THREAD_PER_BLOCK, THREAD_PER_BLOCK, 0, stream >>>(
	    n_grad_input,
	    kernel_size,
	    kernel_bank.data<float>(),
	    make_long4(kernel_bank.size(0), kernel_bank.size(1), 1, 1),
	...
	CSKernelConv2D_backward_function_kernel<<< (n_grad_kernel + THREAD_PER_BLOCK - 1) / THREAD_PER_BLOCK, THREAD_PER_BLOCK, 0,stream >>>(
	    n_grad_kernel,
	    kernel_size,
	    input.data<float>(),
	    make_long4(input.size(0), input.size(1), input.size(2), input.size(3)),
