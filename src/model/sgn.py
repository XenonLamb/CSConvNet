...
class SGN(nn.Module):
    def __init__(self, args):
        ...
        # Top subnetwork, K = 3
        self.top1 = Conv2dLayer(1 * (4 ** 3), 32 * (2 ** 3), 3, 1, 1, pad_type = 'reflect', norm = 'none')
        self.top2 = ResConv2dLayer(32 * (2 ** 3), 32 * (2 ** 3), 3, 1, 1, pad_type = 'reflect', norm = 'none')
        self.top3 = Conv2dLayer(32 * (2 ** 3), 32 * (2 ** 3), 3, 1, 1, pad_type = 'reflect', norm = 'none')
        # Middle subnetwork, K = 2
        self.mid1 = Conv2dLayer(1 * (4 ** 2), 32 * (2 ** 2), 3, 1, 1, pad_type = 'reflect', norm = 'none')
        self.mid2 = Conv2dLayer(int(32 * (2 ** 2 + 2 ** 3 / 4)), 32 * (2 ** 2), 3, 1, 1, pad_type = 'reflect', norm = 'none')
        self.mid3 = ResConv2dLayer(32 * (2 ** 2), 32 * (2 ** 2), 3, 1, 1, pad_type = 'reflect', norm = 'none')
        self.mid4 = Conv2dLayer(32 * (2 ** 2), 32 * (2 ** 2), 3, 1, 1, pad_type = 'reflect', norm = 'none')
        # Bottom subnetwork, K = 1
        self.bot1 = Conv2dLayer(1 * (4 ** 1), 32 * (2 ** 1), 3, 1, 1, pad_type = 'reflect', norm = 'none')
        self.bot2 = Conv2dLayer(int(32 * (2 ** 1 + 2 ** 2 / 4)), 32 * (2 ** 1), 3, 1, 1, pad_type = 'reflect', norm = 'none')
        self.bot3 = ResConv2dLayer(32 * (2 ** 1), 32 * (2 ** 1), 3, 1, 1, pad_type = 'reflect', norm = 'none')
        self.bot4 = Conv2dLayer(32 * (2 ** 1), 32 * (2 ** 1), 3, 1, 1, pad_type = 'reflect', norm = 'none')
        # Mainstream
        self.main1 = Conv2dLayer(1, 32, 3, 1, 1, pad_type = 'reflect', norm = 'none')
        self.main2 = Conv2dLayer(int(32 * (2 ** 0 + 2 ** 1 / 4)), 32, 3, 1, 1, pad_type = 'reflect', norm = 'none')
        self.main3 = nn.ModuleList([Conv2dLayer(32, 32, 3, 1, 1, pad_type = 'reflect', norm = 'none')])
        self.main3.append(Conv2dLayer(32, 32, 3, 1, 1, pad_type = 'reflect', norm = 'none'))
        self.main3.append(Conv2dLayer(32, 32, 3, 1, 1, pad_type = 'reflect', norm = 'none'))
        for i in range(2):                            # add m conv blocks
            self.main3.append(Conv2dLayer(32, 32, 3, 1, 1, pad_type = 'reflect', norm = 'none'))
        self.main4 = Conv2dLayer(32, 1, 3, 1, 1, pad_type = 'reflect', norm = 'none')
    ...
    def load_state_dict(self, state_dict, strict=True, reinit=False):
        if not self.reinit:
            ...
            for name, param in state_dict.items():
                ...
                if ((name in own_state) or (name.replace('module.', '') in own_state)) and (
                not any(banname in name for banname in ban_names)):
...
class ResConv2dLayer(nn.Module):
    def __init__(self, in_channels, out_channels, kernel_size, stride=1, padding=0, dilation=1, pad_type='zero',
        ...
        self.conv2d = nn.Sequential(
            Conv2dLayer(in_channels, in_channels, kernel_size, stride, padding, dilation, pad_type, activation,
                        norm,
                        sn),
            Conv2dLayer(in_channels, in_channels, kernel_size, stride, padding, dilation, pad_type, activation,
                        norm,
                        sn),
            Conv2dLayer(in_channels, out_channels, kernel_size, stride, padding, dilation, pad_type,
                        activation='none',
                        norm=norm, sn=sn)
        )
