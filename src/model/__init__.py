...
class Model(nn.Module):
    def __init__(self, args, ckp):
        ...
        if not args.from_scratch:
            self.load(
                ckp.get_path('model', from_legacy=args.from_legacy),
                pre_train=args.pre_train,
                resume=args.resume,
                cpu=args.cpu,
                from_legacy=args.from_legacy,
                reinit = args.reinit, args=args
            )
        #print("after loading:")
        #print(self.model.state_dict().keys())
        self.model = self.model.to(self.device)
        ##TODO: add 9 parallel models here
        ##modules to be copied:
        ##b1,b2,b3,c1,c2,c3
    ...
    def load(self, apath, pre_train='', resume=-1, cpu=False, from_legacy=False, reinit=False, args=None):
        load_from = None
        kwargs = {}
        if from_legacy:
            if cpu:
        ...
        if load_from:
            self.model.load_state_dict(load_from, strict=False, reinit=reinit)
